...
void save_image_to_png(XImage *i, FILE *fp, int thumb) {
    ...
    png_set_IHDR(png, info, i->width / (thumb ? 16 : 1), i->height / (thumb ? 16 : 1), 8, PNG_COLOR_TYPE_RGB, PNG_INTERLACE_NONE, PNG_COMPRESSION_TYPE_DEFAULT, PNG_FILTER_TYPE_BASE);
    png_write_info(png, info);

    png_bytep row = (png_bytep)malloc((i->width / (thumb ? 16 : 1)) * 3 * sizeof(png_byte));
    if (!row) {
        png_destroy_write_struct(&png, &info);
        handle_error("Failed to allocate memory for row", 1);
    }
    for (int y = 0; y < i->height / (thumb ? 16 : 1); y++) {
        png_bytep ptr = row;
        for (int x = 0; x < i->width / (thumb ? 16 : 1); x++) {
            unsigned long pixel = XGetPixel(i, x * (thumb ? 16 : 1), y * (thumb ? 16 : 1));
            *ptr++ = (pixel >> 16) & 0xFF;
            *ptr++ = (pixel >> 8) & 0xFF;
            *ptr++ = pixel & 0xFF;
        }
...
void save_thumbnail_image(XImage *i) {
    ...
    if (!fp) handle_error("Failed to open file for writing thumbnail", 1);
...
void copy_to_clipboard(XImage *i) {
    ...
    if (!fp) handle_error("Failed to open pipe to xclip", 1);
...
void screenshot_selected_area(Display *d) {
    ...
    int start_x = x1 < x2 ? x1 : x2, start_y = y1 < y2 ? y1 : y2;
...
int main(int argc, char *argv[]) {
    ...
    if ((now_flag && (delay > 0 || active_flag || select_flag)) ||
        (delay > 0 && (active_flag || select_flag)) ||
    ...
    else if (delay > 0) { sleep(delay); screenshot_whole_screen(display); }
